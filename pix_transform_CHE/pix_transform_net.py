...
class PixScaleNet(nn.Module):
    ...
    def forward(self, inputs, mask=None, name=None, predict_map=False, forward_only=False):
        ...
        # mask = None ## mask auf none gsetzt
        ...
        if self.pop_target:
            ...
            data = inputs[:,1:,:,:] # alles ausser buildings
        ...
        if self.pop_target:
            ...
            occrate = self.occrate_layer(feats) # occupnacy rate for every value
            #occrate.cpu()
            if self.bayesian:
                if self.pred_var:
        ...
        data = data.cpu() ## data.shape [13, X] X steht für Population im sample
        # plot_2dmatrix(occrate[0][0].cpu().detach().numpy()) to see the occrate or pop_est
        ...
        if self.exptransform_outputs:
            ...
            pop_est = pop_est.exp() # wird nicht ausgeführt
        ...
        if mask is not None: 
            if self.bayesian:
                ...
                return pop_est[0,mask].sum().cpu()  # dieser teil wird ausgeführt
