...
def get_dataset(dataset_name, params, building_features, related_building_features):
    ...
    # configure pathscd /
    rst_wp_regions_path = cfg.metadata[dataset_name]["rst_wp_regions_path"] # admin regions vo wp: hani
    preproc_data_path = cfg.metadata[dataset_name]["preproc_data_path"] # bruchi ned für CH

    # Read input data
    input_paths = cfg.input_paths[dataset_name] # hani
    no_data_values = cfg.no_data_values[dataset_name] # hani usem config file

    ### fine regions erstmal nicht berücksichtigen 
    with open(preproc_data_path, 'rb') as handle: ## bruchi ned
        pdata = pickle.load(handle)

    cr_census_arr = pdata["cr_census_arr"] # siehe file "coarse_census_CH_test" -> mit import statement; hani
    valid_ids = pdata["valid_ids"] # für fine regions alle valid ids
    no_valid_ids = pdata["no_valid_ids"]  # fine_census regionen ohne zugehörigkeit im coarse census
    print("no_valid_ids {}".format(no_valid_ids))
    id_to_cr_id = pdata["id_to_cr_id"] ## hat länge des fine_census und hat als index vom fine census die referenz zum coarse census
    fine_census = pdata["valid_census"] ## dict mit Anzahl Elementen des fine census; jeder Eintrag ist pixelweise; aber nur für valid_ids
    num_coarse_regions = pdata["num_coarse_regions"] ## len(cr_census_arr)
    geo_metadata = pdata["geo_metadata"] ## meta daten eines schweizer tiff; hani
    areas = pdata["areas"] ## ????
    print(rst_wp_regions_path)
    fine_regions = gdal.Open(rst_wp_regions_path).ReadAsArray().astype(np.uint32) ## array aus allen regionen; kann man auch auf coarse anwenden
    ...
    map_valid_ids = create_map_of_valid_ids(fine_regions, no_valid_ids) # TODO

    # Get map of coarse level regions
    cr_regions = compute_map_with_new_labels(fine_regions, id_to_cr_id, map_valid_ids) # TODO

    # Compute area of coarse regions
    cr_areas = compute_grouped_values(areas, valid_ids, id_to_cr_id) # TODO
